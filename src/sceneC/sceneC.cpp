...
#include "../utils/decision-tree-learning/decision-tree-learning.hpp"
...
class ParentDecisionMakingNode : public Decision {
    public:
        ...
        MakeMonsterSpin makeMonsterSpin;
        MakeCharacterReset makeCharacterReset;
        IsCharacterInHostileZone isCharacterInHostileZone;
        PathfindToCharacter *pathfindToCharacter;
        PathfindToPatrolPoint *pathfindToPatrolPoint;

        ParentDecisionMakingNode(Engine *engine, std::vector<sf::Vector2f*> *patrolPoints) {
            this->pathfindToCharacter = new PathfindToCharacter(engine);
            this->pathfindToPatrolPoint = new PathfindToPatrolPoint(engine, patrolPoints);
        }

        void *run(EnvironmentParameters *parameters) {
            AbstractDMNode *node = (AbstractDMNode*) this->getTruthNode()->run(parameters);

            if (node->name == "Pathfinding to patrol point") {
                return pathfindToPatrolPoint->run(parameters);
            } else if (node->name == "Pathfinding to character") {
                return pathfindToCharacter->run(parameters);
            } else if (node->name == "Checking if character is invisible") {
                return isCharacterInvisible.run(parameters);
            } else if (node->name == "Making character reset") {
                return makeCharacterReset.run(parameters);
            } else if (node->name == "Making character invisible") {
                return makeCharacterInvisible.run(parameters);
            } else if (node->name == "x" || node->name == "y" || node->name == "characterX" || node->name == "characterY" || node->name == "obstacleTop" || node->name == "obstacleRight" || node->name == "obstacleLeft" || node->name == "obstacleBottom") {
                return pathfindToPatrolPoint->run(parameters);
            }
...
// Constructor for SceneC, which sets up the scenes entities
SceneC::SceneC() {
    srand(time(NULL));

    std::vector<StatePacket> packets;
    readStatePacketsFromCSV("./assets/monster-states.csv", &packets);

    std::vector<std::string> attributes;
    attributes.push_back("action");
    attributes.push_back("x");
    attributes.push_back("y");
    attributes.push_back("characterX");
    attributes.push_back("characterY");
    attributes.push_back("obstacleTop");
    attributes.push_back("obstacleRight");
    attributes.push_back("obstacleBottom");
    attributes.push_back("obstacleLeft");
    ...
    DecisionTree createdTree;

    ParentDecisionMakingNode parentDecision = ParentDecisionMakingNode(&engine, &patrolPoints);
    Vertex<AbstractDMNode*> *parentDecisionNode = createdTree.addRoot(&parentDecision);

    DecisionMulti rootMulti = DecisionMulti();
    Vertex<AbstractDMNode*> *rootMultiNode = createdTree.insertNode(&rootMulti, parentDecisionNode, true);

    makeDecisionTree(packets, attributes, rootMultiNode, &createdTree);
    ...
    monster.setDecisionMakingTree(&createdTree);
